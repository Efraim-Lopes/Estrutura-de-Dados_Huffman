void freq_count(void *file, hash_table *ht){
    ...
    buffer = (unsigned char *) malloc(sizeof(char) * file_size); // Aloca memória para o buffer.
    fread(buffer, 1, file_size, file); // Lê o arquivo para o buffer.
    for(i = 0; i < file_size; i++){ // Percorre o buffer e atualiza a frequência de cada caractere na tabela de hash.
...
void fprint_tree_bytes_header(void *file, node *huffman_tree){
    // Esta função escreve a representação pré-ordem da árvore de Huffman no arquivo, usando \* para folhas de escape.
    if(huffman_tree != NULL){
        if( (*(unsigned char*) huffman_tree->item == 42 || *(unsigned char*) huffman_tree->item == '\\') && is_leaf(huffman_tree)){
...
void int_bin(char *bin, int num, int bits){
    // Esta função converte um número decimal em sua representação binária em uma string de bits.
    int i;
    for(i=0; i<bits; i++){
...
int write_compressed_file(void *source_file, void *compressed_file, hash_table *ht){
    // Esta função escreve o arquivo comprimido usando o mapeamento de bits da tabela de hash.
    ...
    while(fscanf(source_file, "%c", &byteread) > 0){
        bitshuff = get_char_new_map(ht, byteread);
...
void fprint_trash_and_tree_size(FILE *file, char *bits){
